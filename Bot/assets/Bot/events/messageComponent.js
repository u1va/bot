...
const membersTokens = await client.db.tokens.countDocuments({
    limitServer: true,
    nitrada: false,
    badtoken: false
  });
...
let product, stock, price;

switch (selectedValue) {
    case '1m':
        product = 'Impulsos Mensal';
        stock = oneMonthTokens;
        price = client.config.price_1m;
        break;
    case '3m':
        product = 'Impulsos Trimensal';
        stock = threeMonthsTokens;
        price = client.config.price_3m;
        break;
    case 'n1m':
        product = 'Nitrada Mensal';
        stock = oneMonthTokens;
        price = client.config.price_nitrada_1m;
        break;
    case 'n3m':
        product = 'Nitrada Trimensal';
        stock = threeMonthsTokens;
        price = client.config.price_nitrada_3m;
        break;
    case 'members':
        product = 'Membros Offlines';
        stock = membersTokens;
        price = client.config.price_members;
        break;
    default:
        return menuInteraction.editReply({
            content: `:x: <@!${menuInteraction.user.id}> Opção inválida.`,
            ephemeral: true
        });
}
    ...
    return menuInteraction.editReply({
        ...
        const cartEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Produto Final', value: `2x ${product}`, inline: true },
        ...
        cartCollector.on('collect', async (buttonInteraction) => {
            ...
            if (buttonInteraction.customId === 'save_changes') {
                ...
                updatedEmbed.addFields(
                    { name: 'Produto', value: `${product}`, inline: true },
                    { name: 'Produto Final', value: `${currentQuantity * 2}x ${product}`, inline: true },
                    { name: 'Estoque Disponível', value: `${stock}x`, inline: true },
    ...
    try {
        ...
        const paymentEmbed = new EmbedBuilder()
            ...
            .setDescription(
                ...
                `**Produto:** ${product}\n` +
                `**Produto Final:** ${currentQuantity * 2}x ${product}\n` +
                `**Quantidade:** ${currentQuantity}x\n` +
                `**Valor Total:** R$${(price * currentQuantity).toFixed(2)}\n\n` +
